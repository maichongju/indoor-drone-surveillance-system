...
from general.enum import Enum, IntEnum
...
from log.dronelogger import (BatteryLogger, CoreLogger, LocoLogger,
                             MotorThrustLogger, MultiRangerLogger,
                             PositionLogger)
...
class FlyControlDistance:
    ...
    hover_correction_max_distance: VariableCallback = field(
        default_factory=lambda: VariableCallback(Position(0.05, 0.05, 0.05)))
...
class FlyControlThread(Thread):
        ...
        while True:
            try:
                ...
                if isinstance(command, FlyCommandManually):
                    ...
                    print('Changed to target mode')
                elif isinstance(command, Path):
                    print('Changed to path mode')
                ...
                if self.fly_status == FlyStatus.FLYING:
                    ...
                    # Calculate the distance between the holding position and the current position
                        ...
                        motion = self.get_hover_velocity(
                            ...
                            motion=None if self._fly_control.fly_mode == FlyMode.HOVER else motion)
    ...
    def get_hover_velocity(self,
        ...
        vx, vy = rotate_axis_coord(
            correct_velocity.vx, correct_velocity.vy, -self._drone_state.yaw)
    ...
    def _get_next_movement(self) -> Motion:
       ...
       # Correct yaw if not facing the direction
        ...
        if self._go_to_helper.action == GoToAction.AXIS_CHANGING:
            ...
            yaw = self._get_yaw(self._go_to_helper.moving_direction.axis,
                                dist_to_target,
                                current_yaw,
                                yaw_margin,
                                max_yaw)
            ...
            # Ensure the the average of yaw
            if self._go_to_helper.yaw_buffer.avg() == 0:
                self._go_to_helper.action = GoToAction.HOLD
                self._go_to_helper.next_action = GoToAction.MOVING
                self._go_to_helper.hold_position = self.hover_position
        ...
        if self._go_to_helper.action == GoToAction.MOVING:
            ...
            if self._go_to_helper.moving_direction.axis == Axis.X:
                ...
                if thrust_percent == 0 or self._is_pass_target(
                    ...
                    # !Test
                    self._go_to_helper.hold_position = self._drone_state.position
                    self._go_to_helper.next_action = GoToAction.REQUIRE_AXIS_CHANGE

                # if thrust_percent == 0:
                #     LOGGER.debug('Reach x position')
                #     self._go_to_helper.reach_x = True
                #     self.setting.fly_mode.set(FlyMode.HOVER)
            ...
            # Moving alone with Y axis
                ...
                # if thrust_percent == 0:
                #         self._go_to_helper.action = GoToAction.REQUIRE_AXIS_CHANGE
                #         LOGGER.debug('reach y change to x')
                #         self._set_maintain_direction(False)
                #         self._go_to_helper.reach_y = True
                ...
                if thrust_percent == 0 or self._is_pass_target(
                    ...
                    motion.vx = vx
            # motion.vx, motion.vy = rotate_axis_coord(
            #     motion.vx, motion.vy, -current_yaw)
    ...
    def hover_set(self) -> bool:
        return self.hover_position.x != None and self.hover_position.y != None and self.hover_position.z != None
