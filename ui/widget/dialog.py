...
from ui.widget.location import LocationItem, LocationListWidget
...
class GoToDialog(QDialog):
    ...
    def _add_btn_click_callback(self):
        if (self._validate_input()):
            ...
            location = Location(name="", position= pos)
...
class LocationDialog(QDialog):
    ...
    def show_edit_dialog(self, item:LocationItem):
        ...
        if value == QDialog.DialogCode.Accepted:
            item.update()
            self.locations_list.locations.update_location(old_location_name, dialog.location.name, dialog.location.position)
    ...
    def _btn_ok_on_click(self):
        ...
        if self._is_update:
            ...
            if LOCATIONS.existed(self.le_name.text()):
                self.lbl_error.setText("Name already existed")
                return
            self.location = Location(self.le_name.text(), Position(float(self.le_x.text()), float(self.le_y.text()), float(self.le_z.text())))            
